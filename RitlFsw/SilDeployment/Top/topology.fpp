module RitlFsw {
  ...
  # ----------------------------------------------------------------------
    ...
    instance comDriver
    instance cmdSeq
    instance orchReceiver
    instance flightMain
    instance sensorServer
    instance sensorBufferManager
    instance orchSender
  ...
  # ----------------------------------------------------------------------
    ...
    # include "SilDeploymentPackets.fppi"
  ...
  # ----------------------------------------------------------------------
    ...
    connections RateGroups {
      ...
      rateGroup3.RateGroupMemberOut[5] -> sensorBufferManager.schedIn
      rateGroup3.RateGroupMemberOut[6] -> orchSenderBufferManager.schedIn
    }
    ...
    connections SensorDataInPipeline {
      ...
      # TcpServer -> OrchReceiver: deliver received bytes
      sensorServer.$recv -> orchReceiver.DataIn

      # OrchReceiver -> TcpServer: send ACK back to orchestrator
      orchReceiver.DataOut -> sensorServer.$send

      # OrchReceiver returns the receive buffer to the TcpServer,
    ...
    connections ActuationDataOutPipeline {
      ...
      # orchSenderClient receive buffer management (reuse same pool)
      orchSenderClient.allocate   -> orchSenderBufferManager.bufferGetCallee
      orchSenderClient.deallocate -> orchSenderBufferManager.bufferSendIn

      # Actuation to TCP client
      orchSender.dataOut -> orchSenderClient.$send
    ...
    connections SilDeployment {
      ...
      flightMain.sensorDatatoRecovery-> recoveryController.sensorDataIn
